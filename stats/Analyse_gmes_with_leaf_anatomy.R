...
setwd("C:/R-projects/EucFACE_gmes/rawdata")
#read in datafile
anatomy<-read.csv("C:/R-projects/EucFACE_gmes/rawdata/face_gm_anatomy2-corr.csv") #cross section tab
...
# R6: there were 2 trees outside the ring, but one set had just O and it was not clear which tree they belonged too, this data got deleted in the anatomy-corr files
# All name conventions are now also named consistently across files which will help with concatanating and merging.
...
anatomy2<-read.csv("C:/R-projects/EucFACE_gmes/rawdata/face_gm_anatomy3-corr.csv") # leaf measurements tab
...
anat2.tree<-summaryBy(leafw+ epiupper+ epilower+ mesolay+ meso+ length.par1+ stomdenad+ stomdenab + stomlengthad + stomlengthab + spi.ad + spi.ab + sumlength.par + meanlength.par  ~ Tree +Position, data=anatomy2, FUN = mean)
...
phys<-read.csv("C:/R-projects/EucFACE_gmes/rawdata/face_leaves_with_gmes.csv")
...
full.data2<-merge(anat2.tree, phys.CO2, all=TRUE, by="Conc") # note that I just used anat2 here to avoid repetition
names(full.data2)[2:8]<-c("Tree", "Position", "leafw", "epi_up", "epi_low", "mesolay.mean", "meso.mean")
names(full.data2)[25]<-"gmes"

# if we want to include the measurement CO2 we can merge, but will double up the anatomy means data
full.data1<-merge(anat2.tree, phys, all=TRUE, by="Conc")
names(full.data1)[2:8]<-c("Tree", "Position", "leafw", "epi_up", "epi_low", "mesolay.mean", "meso.mean")
names(full.data1)[25]<-"gmes"

write.csv(full.data1,"C:/R-projects/EucFACE_gmes/rawdata/Physiology-anatomy-data-corr.csv")

# can merge LMA and leaf nitrogen concentrations (but not for all leaves) - done manually and redone on 04/07/2018
